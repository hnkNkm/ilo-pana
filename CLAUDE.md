...
- **`internal/`**: Core business logic, not importable by external packages
  - **`config/`**: Command-line flag parsing and configuration management. Handles multiple `-H` flags using a custom header-slice type that implements `flag.Value` interface
...
1. **Flag handling**: Uses `flag.Var()` with a custom header-slice type to support multiple header flags
2. **Sensitive data protection**: Automatic masking of sensitive headers unless `-v` (verbose) is passed
...
- The header-slice type used for multiple `-H` flags lives in `internal/config/config.go`
- URL validation (`request.ValidateURL` in `internal/request/request.go`) only warns on localhost/private IPs; it does NOT block them
